...
def test_every_real_incorporation_wording_is_recognised(key, recorded_sections):
    assert is_incorporated_by_reference(Section.MARKET_RISK, recorded_sections[key])
...
def test_a_real_item_7_8_boundary_miss_is_caught(recorded_sections, recorded_filing):
    ...
    assert not is_incorporated_by_reference(
        Section.MARKET_RISK, recorded_sections["gm_item_7a_spill"]
    )
...
def test_a_real_section_is_not_mistaken_for_a_pointer(recorded_filing):
    ...
    assert not is_incorporated_by_reference(
        Section.MARKET_RISK, recorded_filing.sections[Section.MARKET_RISK]
    )
